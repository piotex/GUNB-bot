...
def get_documents_data(driver: WebDriver, url:str, data_model: DataModel) -> XPathModel3:
    ...
    return model3
def get_documents_data2(driver: WebDriver, url:str, data_model: DataModel) -> XPathModel3:
    driver.get(url)
    solve_captcha_if_visible(driver, data_model)

    model3 = XPathModel3()
    model3_dict = model3.__dict__
    last_i = 0
    for row in range(1,50):
        for dict_key in model3_dict:
            try:
                x_path_sel = f"/html/body/div/div[2]/div/div[2]/div[{i}]/div[1]"
                x_path_val = f"/html/body/div/div[2]/div/div[2]/div[{i}]/div[2]/p"
...
def main():
    ...
    model3 = get_documents_data2(driver, url, model)
